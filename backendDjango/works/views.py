...
def helper_merge_work_and_options(query_set_of_work_models,
                                  query_set_of_work_options):
    return {
        'application_name':
            query_set_of_work_models.application_name,
        'application_description':
            query_set_of_work_models.application_description,
        'application_picture_url':
            query_set_of_work_models.application_picture_url,
        'application_options': list(query_set_of_work_options.values(
                                    'icon_name', 'contents_url',
                                    'icon_image_path'))
    }
...
def get_portfolio_work(request, id):
    work = Work.objects.prefetch_related(
        Prefetch('application_options',
                 queryset=WorkOption.objects.filter(work_id=id)))
